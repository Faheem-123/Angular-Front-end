...
@Component({
  selector: 'lagged-collection-report',
  templateUrl: './lagged-collection-report.component.html',
  styleUrls: ['./lagged-collection-report.component.css']
})
export class LaggedCollectionReportComponent implements OnInit {
...
debugger;
  ...
  onSearch(){
    ...
    this.reportsService.getLaggedCollectionReport(providerWiseCollSearch).subscribe(
      data => {
        debugger;

        this.lstlaggedCollection =  data;   

        this.lstHeader.push(//payer_id
          {
            header:""
          });
          // this.lstHeader.push(//payer_name
          //   {
          //     header:""
          //   });
        ...
        for (let i: number = this.lstlaggedCollection.length - 1; i >= 0; i--) {
          ...
          {
            if(this.lstlaggedCollection[i][0]!='-10000')
              this.lstlaggedCollection.splice(i, 1);
            if(this.lstlaggedCollection[i][0]=='-10000')
              this.lstlaggedCollection[i][1]='Total Contractual';
          }
          else if(this.lstlaggedCollection[i][0]==0)
          {
            ...
            {
              this.lstlaggedCollection[i][2]="";
              this.lstlaggedCollection.splice(0,0,this.lstlaggedCollection[i])
              this.lstlaggedCollection.splice(i+1, 1);
              continue;
            }
            //Botom Summary
        ...
        for(let a=0;a<this.lstCharges.length;a++)
        {
          ...
          //
          this.lstCharges[a][this.lstHeader.length]=Number(total).toFixed(2);
        }
        //Body
        // for(let a=0;a<this.lstlaggedCollection.length;a++)
        // {
        //   if(this.lstlaggedCollection[a][0]=="101010")
        //   {
        ...
        for(let a=0;a<this.lstBottomSummary.length;a++)
        {
          ...
          //
          this.lstBottomSummary[a][this.lstHeader.length]=Number(total).toFixed(2);
        }
debugger;
...
//         }
        ...
        for(let a=0;a<this.lstlaggedCollection.length;a++)
        {
          ...
          //
          this.lstlaggedCollection[a][this.lstHeader.length]=Number(total).toFixed(2);
        }
        debugger;
  ...
  exportAsXLSX(){
    ...
    for(let c=0;c<this.lstHeader.length;c++)
    {
      if(c==0)
      {
        lstColumns.push( new ExcelColumn('payer_id', 'Payer ID'));
      }
