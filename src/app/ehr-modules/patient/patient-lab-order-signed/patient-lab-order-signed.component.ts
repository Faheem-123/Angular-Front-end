...
@Component({
  selector: 'patient-lab-order-signed',
  templateUrl: './patient-lab-order-signed.component.html',
  styleUrls: ['./patient-lab-order-signed.component.css']
})
export class PatientLabOrderSignedComponent implements OnInit {
  @Input() patientId = '';
  @Input() openPatientInfo: OpenedPatientInfo;
  pat_name: string = '';
  lstPendingResults;
  lstPendingAttachment;
  frmSearchSignedResults: FormGroup;
  ...
  constructor(private formBuilder: FormBuilder, private logMessage: LogMessage,
    @Inject(LOOKUP_LIST) public lookupList: LookupList, private labService: LabService, private dateutil: DateTimeUtil
    , private generalService: GeneralService, private generalOperation: GeneralOperation
    , private modalService: NgbModal, private openModuleService: OpenModuleService) { }
  ...
  ngOnInit() {
    ...
    if (this.patientId != '' && this.patientId != null)
      this.onFilter(this.frmSearchSignedResults.value);
  }
  getLabCategoy() {
  ...
  buildForm() {
    ...
    this.frmSearchSignedResults = this.formBuilder.group({
      ...
      //drpProvider: this.formBuilder.control("50010112", Validators.required),
      //drpLocation: this.formBuilder.control("50010112", Validators.required),
      // drpProvider: this.formBuilder.control(this.lookupList.logedInUser.defaultProvider.toString()==''?'all':this.lookupList.logedInUser.defaultProvider, Validators.required),
      // drpLocation: this.formBuilder.control(this.lookupList.logedInUser.defaultLocation.toString()==''?'all':this.lookupList.logedInUser.defaultLocation, Validators.required),
      ...
      chkSignedWithAttachmnet: this.formBuilder.control(false)
  ...
  onFilter(criteria) {
    ...
    if (criteria.txtDateTo == null) {
      GeneralOperation.showAlertPopUp(this.modalService, 'Signed Result', "Please Enter To Date", AlertTypeEnum.WARNING);
      return;
    }
    if (criteria.txtDateFrom == null) {
      GeneralOperation.showAlertPopUp(this.modalService, 'Signed Result', "Please Enter From Date", AlertTypeEnum.WARNING);
      return;
    }

    this.isLoading = true;
    if (criteria.chkSignedWithAttachmnet == false) {
      ...
      this.labService.getSignedResults(searchCriteria).subscribe(
        data => {
          debugger;
      ...
      this.labService.getSignedAttachments(searchCriteria).subscribe(
        data => {
          debugger;
          this.isLoading = false;
