...
import { ORMWriteOffCodes } from 'src/app/models/billing/ORMWriteOffCodes';
@Component({
  selector: 'write-off-setup',
  templateUrl: './write-off-setup.component.html',
  styleUrls: ['./write-off-setup.component.css']
})
export class WriteOffSetupComponent implements OnInit {
  inputForm:FormGroup;
  arrCodes;
  selectedIndex=0;
  operation='';
  ...
  getCode() {
    this.setupService.getWriteOffcode(this.lookupList.practiceInfo.practiceId).subscribe(
      data => {
        this.arrCodes = data as Array<any>;
        if(this.arrCodes.length>0)
        {
  ...
  onDelete() {
    ...
    modalRef.result.then((result) => {
      ...
      if (result == PromptResponseEnum.YES) {
        ...
        this.setupService.deleteWriteOffcode(deleteRecordData)
          .subscribe(
  ...
  objSave: ORMWriteOffCodes;
  onSave() {
    this.objSave = new ORMWriteOffCodes;
    ...
    this.setupService.saveWriteOffcode(this.objSave).subscribe(
      data => {
        debugger;
        this.saveCodeSuccess(data); 
      },
      error => {
