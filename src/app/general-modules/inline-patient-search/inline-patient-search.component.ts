import { Component, OnInit, Input, Output, EventEmitter, Inject, ViewChildren, OnChanges } from '@angular/core';
...
@Component({
  selector: 'inline-patient-search',
  templateUrl: './inline-patient-search.component.html',
  styleUrls: ['./inline-patient-search.component.css']
})
export class InlinePatientSearchComponent implements OnChanges {
  ...
  searchPatient() {
    ...
    if (this.searchValue != undefined && this.searchValue != "") {
      ...
      this.searchService.searchPatient(searchCriteria).subscribe(
        data => {
          this.lstPatient = data as Array<any>
          this.isLoading = false;
        },
        error => {
          this.onSearchPatientError(error);
          this.isLoading = false;
        }
      );
    }
